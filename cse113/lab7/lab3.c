 ...
 *
 * @details Finds multiple attributes of the given array
 *
 * @bugs none
 *
 * @todo none
...
int main(void)
{
	...
	max = find_max(a, size);
	min = find_min(a, max, size);
	mid = midpoint(max, min);
	ltmid = get_count(a, size, LT, mid);
	gtmid = get_count(a, size, GT, mid);
	search = linear_search(mid, a, size);
	sum = add(a, size);
	ave = average(a, size);
	med = median(a, size);
	even = count_even(a, size);
	odd = count_odd(a, size);
	divide = divisible_count(divisor, a, size);

	print_summary(size, max, min, mid, ltmid, gtmid, search, sum, ave, med, even, odd, divide);
