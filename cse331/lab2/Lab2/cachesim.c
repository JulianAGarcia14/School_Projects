...
{
    /* Local Variables */
    int write_policy, counter, i, j, k, Lsize, associativity, datasize, rpolicy, mpenalty, writeallocate, lhitrate, shitrate, totalhits;
    Cache cache;
    FILE *file;
    FILE *config;
    char mode, address[100], hold[100];
    float lhr, shr;
    ...
    k=0;
    while( fgets(buffer, LINELENGTH, config) != NULL )
    {
        if(k==0)
        {
                while(buffer[i] != '\n')
        ...
        if(k==1)
        {
                ...
                associativity = atoi(hold);
                printf("Associativity = %d", associativity);
        }
        ...
        if(k==2)
        {
                ...
                datasize = atoi(hold);
                printf("Data size in kb = %d", datasize);
        }
        memset(hold, 0, 100);
        i =0;
        j = 0;
        ...
        if(k==3)
        {
                ...
                rpolicy = atoi(hold);
                printf("Replacement Policy = %d", rpolicy);
        }
        memset(hold, 0, 100);
        i =0;
        j = 0;
        ...
        if(k==4)
        {
                ...
                mpenalty = atoi(hold);
                printf("Miss Penalty = %d", mpenalty);
        }
        memset(hold, 0, 100);
        i =0;
        j = 0;
        ...
        if(k==5)
        {
                ...
                writeallocate = atoi(hold);
                printf("Write allocate = %d", writeallocate);
        }
        memset(hold, 0, 100);
        i =0;
        j = 0;
    ...
    //printf("totalhits: %d \n", totalhits);
    lhitrate = cache->loadhits;
    //printf("loadhits: %d \n", lhitrate);
    shitrate = cache->storehits;
    //printf("storehits: %d \n", shitrate);
    lhr = (float) lhitrate/totalhits*100;
    shr = (float) shitrate/totalhits*100;

    printf("CACHE LOAD HIT RATE: %f \n", lhr);
    printf("CACHE STORE HIT RATE: %f \n", shr);
